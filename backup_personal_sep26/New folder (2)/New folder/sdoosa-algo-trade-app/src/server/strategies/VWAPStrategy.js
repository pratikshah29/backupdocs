...
class VWAPtrategy extends BaseStrategy {
    ...
    return this.fetchCandlesHistory().then(() => {
      ...
      _.each(this.stocks, tradingSymbol => {
        ...
        try {
          ...
          // check change with first candle close
          const firstCandle = data.candles[0];
          const change = percentageChange(firstCandle.close, data.prevDayData.close);
          logger.info(`${this.name}: process: ${tradingSymbol} change = ${change}`);
          if (Math.abs(change) < 0.5) {
            logger.info(`${this.name}: ${tradingSymbol} not qualified as first candle change with prev day close is less than 0.5%`);
  ...
  generateTradeSignals(data, longDecisionCandle, shortDecisionCandle, broker) {
    ...
    let CAPITAL_PER_TRADE, RISK_PERCENTAGE_PER_TRADE, MARGIN = 1;
    if (enableRiskManagement) {
      const totalCapital = parseInt(_.get(this.strategy, 'withRiskManagement.totalCapital', 1000));
      const maxTrades = parseInt(_.get(this.strategy, 'withRiskManagement.maxTrades', 1));
      CAPITAL_PER_TRADE = parseInt(totalCapital / maxTrades);
      RISK_PERCENTAGE_PER_TRADE = parseFloat(_.get(this.strategy, 'withRiskManagement.riskPercentagePerTrade', 0.5));
    ...
    if (longDecisionCandle && !ts1) {
      ...
      if (enableRiskManagement) {
        ts1.quantity = calculateSharesWithRiskFactor(CAPITAL_PER_TRADE, ts1.trigger, ts1.stopLoss, RISK_PERCENTAGE_PER_TRADE);
      } else {
    ...
    if (shortDecisionCandle && !ts2) {
      ...
      if (enableRiskManagement) {
        ts2.quantity = calculateSharesWithRiskFactor(CAPITAL_PER_TRADE, ts2.trigger, ts2.stopLoss, RISK_PERCENTAGE_PER_TRADE);
      } else {
